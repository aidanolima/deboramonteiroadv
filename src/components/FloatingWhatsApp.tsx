...
export default function FloatingWhatsApp() {
  ...
  const [chatLog, setChatLog] = useState([
    { role: 'ai', content: 'Olá! Sou a assistente virtual do escritório Débora Monteiro. Para um atendimento mais rápido, como posso ajudar com o seu caso hoje?' }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ history: newChatLog })
      });

      const data = await response.json();

      if (response.ok) {
        // INTERCEPTADOR DE GATILHOS (A Mágica da Conversão)
        if (data.reply.includes('[REDIRECIONAR_AGENDA]')) {
          setChatLog((prev) => [...prev, { 
            role: 'ai', 
            content: 'GATILHO_AGENDA', // Sinalizador interno
          }]);
        } else {
  ...
  return (
    <>
      {!isOpen && (
        <button onClick={toggleChat} className="fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 bg-amber-600 text-white rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.3)] hover:bg-amber-700 hover:scale-110 transition-all duration-300 group" aria-label="Abrir Atendimento">
      ...
      {isOpen && (
        <div className="fixed bottom-0 right-0 md:bottom-6 md:right-6 z-50 w-full h-[100dvh] md:w-[380px] md:h-[600px] bg-white md:rounded-2xl shadow-2xl flex flex-col border border-zinc-200 overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
          ...
          {/* CABEÇALHO */}
          <div className="bg-zinc-900 p-4 flex items-center justify-between shadow-md border-b border-zinc-700/50">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-amber-600 rounded-full flex items-center justify-center text-white font-serif font-bold">DM</div>
              <div>
                <h3 className="text-white font-bold text-sm">Assistente Virtual</h3>
                <p className="text-amber-500 text-xs flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span> Online agora</p>
              </div>
            </div>
          ...
          <div className="flex-1 bg-zinc-50 p-4 overflow-y-auto flex flex-col gap-4">
            ...
            {chatLog.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {/* SE FOR O GATILHO DA AGENDA, RENDERIZA UM BOTÃO DE REDIRECIONAMENTO */}
                {msg.content === 'GATILHO_AGENDA' ? (
                  <div className="max-w-[90%] p-4 text-sm bg-white border border-amber-200 text-zinc-800 shadow-lg rounded-2xl rounded-tl-none flex flex-col gap-3">
                    <p className="font-medium">Perfeito! A Dra. Débora está pronta para te atender.</p>
                    <a href="#contato" onClick={() => setIsOpen(false)} className="bg-amber-600 hover:bg-amber-700 text-white text-center font-bold py-3 px-4 rounded-lg transition-colors shadow-md animate-pulse">
                      IR PARA A AGENDA AGORA
                    </a>
                  </div>
                ) : (
                  /* MENSAGEM NORMAL */
                  <div className={`max-w-[85%] p-3 text-sm rounded-2xl ${msg.role === 'user' ? 'bg-amber-600 text-white rounded-tr-none' : 'bg-white border border-zinc-200 text-zinc-800 shadow-sm rounded-tl-none whitespace-pre-wrap'}`}>
                    {msg.content}
                  </div>
                )}
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-zinc-100 flex gap-2">
            ...
            />
            <button type="submit" disabled={!mensagem.trim() || isLoading} className="bg-amber-600 hover:bg-amber-700 disabled:bg-zinc-300 text-white rounded-full w-12 h-12 flex items-center justify-center transition-colors shadow-sm">
